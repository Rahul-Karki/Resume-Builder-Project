...
const generateAccessToken = (userId: string, tokenVersion = 0) => {
  ...
  if (hasHmac) {
    return jwt.sign({ userId, tokenVersion }, env.JWT_ACCESS_SECRET, { algorithm: "HS256", expiresIn: "15m" });
  }

  if (hasRsa) {
    const { privateKey } = getAccessKeyPair();
    logger.info("Using RSA keys to sign access token");
    return jwt.sign({ userId, tokenVersion }, privateKey, { algorithm: "RS256", expiresIn: "15m" });
  }
...
const generateRefreshToken = (userId: string, tokenVersion = 0) => {
  ...
  if (hasHmac) {
    return jwt.sign({ userId, tokenVersion }, env.JWT_REFRESH_SECRET, { algorithm: "HS256", expiresIn: "7d" });
  }

  if (hasRsa) {
    const { privateKey } = getRefreshKeyPair();
    logger.info("Using RSA keys to sign refresh token");
    return jwt.sign({ userId, tokenVersion }, privateKey, { algorithm: "RS256", expiresIn: "7d" });
  }

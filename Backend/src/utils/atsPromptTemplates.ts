...
const DEFAULT_ENHANCED_ATS_SYSTEM_PROMPT = [
  ...
  "4. Every suggestion must specify exact location, explain WHY it hurts ATS score, provide improved replacement content, and ESTIMATE POINTS GAINED toward the overall ATS score.",
  ...
  "",
  "IMPORTANT RULES:",
  "- Never invent experience, employers, tools, years, or degrees — if the resume has no experience section, suggest adding one with real entries, do NOT create fake ones.",
  "- Every suggestion must include clickToApply payload with exact targetText and replacementText.",
...
const DEFAULT_ENHANCED_ATS_SCORING_PROMPT = [
  ...
  "3) Every actionable suggestion MUST include clickToApply with targetText and replacementText so the frontend can directly apply changes.",
  "4) If JOB_DESCRIPTION is empty, infer target role keywords from resume and still provide improvement suggestions.",
  "5) Never hallucinate fake experience. Only improve wording and structure.",
  "6) Every suggestion must include a realistic atsGain (0-15 points) estimating score increase.",
  "7) Order suggestions by atsGain descending — highest impact first.",
  "8) Each suggestion must reference which ATS category it improves (keywordMatch, parsing, contentQuality, experienceRelevance, formatting).",
  "",

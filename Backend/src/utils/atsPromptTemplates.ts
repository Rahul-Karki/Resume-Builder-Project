...
const DEFAULT_ENHANCED_ATS_SCORING_PROMPT = [
  ...
  "- Every missing keyword MUST include a `keyword_placement` entry showing exactly where and how to insert it.",
  "- Every formatting issue MUST include a concrete `fix` in `formatting_fixes` with exact CSS/content changes.",
  "- Every action_plan item MUST have `how_to_do` with 2-5 specific, executable steps the user can follow.",
  "- Every section_audit item with status 'missing' or 'weak' MUST have a `fix.copy_paste_template` with ready-to-use text.",
  "",
  "KEYWORD BUCKETS",
  ...
  '         "fix": {',
  '            "why": "exactly what is wrong and why it hurts the score",',
  '            "keywords_to_include": ["keyword1", "keyword2"],',
  '            "copy_paste_template": "ready-to-copy-paste template text the user can directly drop into their resume",',
  '            "example": "full concrete example showing the fixed version",',
  ...
  '      "keyword_placement": [',
  '         { "keyword": "", "place_in": ["summary", "skills", "experience", "projects"], "example_usage": "A complete sentence or phrase showing exactly how to use this keyword in context" }',
  "      ]",
  "   },",
  '   "rewrite_suggestions": [',
  "      {",
  ...
  '            "type": "bullet_improvement|summary_rewrite|skill_add|grammar_fix|keyword_insertion|quantify|action_verb_improvement|formatting_fix",',
  ...
  "      {",
  '         "id": "fmt-1",',
  '         "issue": "description of the formatting problem",',
  '         "fix": "exact step-by-step fix the user should apply",',
  '         "expected_impact": "how many points this fix will recover",',
  '         "code_example": "showing the corrected formatting or structure if applicable"',
  "      }",
  "   ],",
  '   "action_plan": [',
  "      {",
  '         "priority": "P0|P1|P2",',
  '         "action": "clear one-line action title",',
  '         "why_it_increases_score": "explanation of how this directly improves the ATS score",',
  '         "how_to_do": ["Step 1: ...", "Step 2: ...", "Step 3: ..."],',
  '         "expected_score_gain": 0',
  "      }",
  "   ],",
  '   "quick_wins": ["actionable one-sentence quick fix the user can apply right now"],',
  ...
  "}",
  "",
  "CRITICAL: ALL suggestions must be directly applicable — the user should be able to copy-paste your `after` text, follow your `how_to_do` steps, or click 'Apply' to use `auto_apply_payload`. Never just flag a problem without providing the fix.",
].join("\n");

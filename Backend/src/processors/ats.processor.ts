...
const normalizeSectionWiseAnalysis = (value: unknown): SectionWiseAnalysis[] => {
  ...
  return raw.filter((item) => item && typeof item === "object").map((item) => {
    ...
    return {
      section: compactText(record.section) as ExpandedSectionKey,
...
const normalizeWarnings = (value: unknown): AtsWarning[] => {
  ...
  return raw.filter((item) => item && typeof item === "object").map((item) => {
    ...
    return { type: (t === "missing_section" || t === "empty_section" || t === "weak_content" || t === "missing_field" || t === "formatting" || t === "keyword" || t === "experience" || t === "education") ? t : "weak_content", severity: (sv === "critical" || sv === "warning" || sv === "info") ? sv : "warning", section: compactText(record.section) as ExpandedSectionKey, message: compactText(record.message), suggestion: compactText(record.suggestion) } as AtsWarning;
  });
};

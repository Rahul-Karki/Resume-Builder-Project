...
const CombinationTemplate = memo(function CombinationTemplate({ data }: { data: ResumeDocument }) {
          ...
          {contacts.length > 0 ? (
            <div className="cmb-contact" style={{ justifyContent: style.headerAlign === "center" ? "center" : "flex-start" }}>
              {contacts.map((item, index) => (
                <span className="cmb-contact-item" key={`${item.label}-${index}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                  {item.isLink ? (
                    <a className="cmb-link" href={item.href} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>

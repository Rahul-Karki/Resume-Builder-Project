          ...
          {contacts.length > 0 ? (
            <div className="cim-contact" style={{ justifyContent: style.headerAlign === "center" ? "center" : "flex-start" }}>
              {contacts.map((item, index) => (
                <span className="cim-contact-item" key={`${item.label}-${index}`}>
                  {item.href ? <a className="cim-link" href={item.href} target="_blank" rel="noreferrer">{item.label}</a> : item.label}
                </span>
              ))}
            </div>
          ) : null}

  ...
  return (
    <div style={{ padding: isMobile ? "20px 12px" : "28px 32px", fontFamily: "'Outfit', sans-serif", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 20, flexWrap: "wrap", flexDirection: isMobile ? "column" : "row" }}>
        ...
        <div style={{ position: "relative", flex: 1, minWidth: 200, maxWidth: isMobile ? "100%" : 280, width: isMobile ? "100%" : "auto" }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search templates…"
            style={{ width: "100%", padding: "7px 12px 7px 30px", background: "#18181b", border: "1px solid #3f3f46", borderRadius: 8, color: "#d4d4d8", fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
            onFocus={e => e.currentTarget.style.borderColor = "#71717a"}
            onBlur={e => e.currentTarget.style.borderColor = "#3f3f46"}
          />
          {search && <button onClick={() => setSearch("")} style={{ position: "absolute", right: 9, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "#71717a", cursor: "pointer", fontSize: 14 }}>×</button>}
        </div>
        ...
        {(["all", "published", "draft", "archived"] as FilterStatus[]).map(s => (
          <button key={s} onClick={() => setFilterStatus(s)} style={{
            ...
            borderColor: filterStatus === s ? "#C8F55A" : "#3f3f46",
            background: filterStatus === s ? "rgba(200,245,90,0.1)" : "transparent",
            color: filterStatus === s ? "#C8F55A" : "#a1a1aa",
        ...
        <select value={filterCategory} onChange={e => setFilterCategory(e.target.value as FilterCategory)}
          style={{ padding: "6px 10px", background: "#18181b", border: "1px solid #3f3f46", borderRadius: 8, color: "#a1a1aa", fontSize: 12, fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
          <option value="all">All Categories</option>
          <option value="non-tech">Non-Tech</option>
          <option value="tech">Tech</option>
        </select>

        <span style={{ fontSize: 12, color: "#71717a", marginLeft: isMobile ? 0 : "auto" }}>
          {filtered.length} result{filtered.length !== 1 ? "s" : ""}
        </span>
      </div>

      {loading && <SkeletonTemplateGrid />}

      {/* Error */}
      {error && (
        <div style={{ padding: "16px 20px", background: "rgba(220,38,38,0.1)", border: "1px solid rgba(220,38,38,0.3)", borderRadius: 10, color: "#fca5a5", fontSize: 13 }}>
          {error}
        </div>
      )}
      ...
      {!loading && !error && (
        filtered.length > 0 ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: "#a1a1aa", marginBottom: 6 }}>
              {search ? `No templates match "${search}"` : "No templates found"}
            </div>
            <button onClick={() => { setSearch(""); setFilterStatus("all"); setFilterCategory("all"); }}
              style={{ background: "none", border: "none", color: "#71717a", fontSize: 13, cursor: "pointer", fontFamily: "inherit", textDecoration: "underline" }}>
              Clear filters
            </button>
          </div>
        )
      ...
      {previewTarget && (
        ...
        >
          <div style={{ width: "100%", maxWidth: 1440, height: isMobile ? "calc(100vh - 20px)" : "calc(100vh - 48px)", background: "#18181b", border: "1px solid #3f3f46", borderRadius: 20, overflow: "visible", boxShadow: "0 40px 100px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.05)", marginTop: isMobile ? 0 : 8, display: "flex", flexDirection: "column" }}>
            {/* Header with gradient background */}
            <div style={{ background: "linear-gradient(135deg, rgba(200,245,90,0.08) 0%, rgba(200,245,90,0.02) 100%), #18181b", borderBottom: "1.5px solid #3f3f46", display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "14px 12px" : "24px 28px", position: "sticky", top: 0, zIndex: 10, gap: 12, flexShrink: 0 }}>
              <div>
                <div style={{ fontSize: 18, fontWeight: 800, color: "#fafafa", letterSpacing: "-0.5px" }}>{previewTarget.name}</div>
                <div style={{ fontSize: 12, color: "#8AA0FF", marginTop: 6, fontWeight: 500 }}>
                  <span style={{ color: "#71717a" }}>layoutId: </span>{previewTarget.layoutId} · <span style={{ color: "#71717a" }}>Category: </span>{previewTarget.category} · <span style={{ color: "#71717a" }}>Tags: </span>{(previewTarget.tags?.length ? previewTarget.tags : [previewTarget.tag]).join(", ")}
                </div>
              </div>
              <button
                onClick={closePreview}
                onMouseEnter={e => { e.currentTarget.style.background = "#27272a"; e.currentTarget.style.transform = "scale(1.1)"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "#18181b"; e.currentTarget.style.transform = "scale(1)"; }}
                style={{ width: 38, height: 38, borderRadius: 12, border: "1.5px solid #3f3f46", background: "#27272a", color: "#d4d4d8", cursor: "pointer", fontSize: 20, fontWeight: 700, transition: "all 0.2s ease" }}
              >
                ×
              </button>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "320px minmax(0, 1fr)", minHeight: 0, flex: 1 }}>
              {/* Sidebar */}
              <div style={{ padding: isMobile ? 14 : 28, borderRight: isMobile ? "none" : "1.5px solid #3f3f46", borderBottom: isMobile ? "1.5px solid #3f3f46" : "none", background: "linear-gradient(180deg, #27272a 0%, #18181b 100%)", overflow: "auto", minHeight: 0 }}>
              ...
              {/* Preview */}
              <div style={{ overflow: "auto", background: "linear-gradient(135deg, #18181b 0%, #09090b 100%)", display: "flex", justifyContent: "center", alignItems: "flex-start", padding: isMobile ? 12 : 24, minHeight: 0 }}>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.type === "error" ? "rgba(220,38,38,0.15)" : "#27272a",
          color: toast.type === "error" ? "#fca5a5" : "#fafafa",
          padding: "10px 22px", borderRadius: 24, fontSize: 13, fontWeight: 600,
          border: `1px solid ${toast.type === "error" ? "rgba(220,38,38,0.3)" : "#3f3f46"}`,
...
function InfoRow({ label, value, isHighlight = false, showColor }: { label: string; value: string; isHighlight?: boolean; showColor?: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <div style={{ fontSize: 10, fontWeight: 800, color: isHighlight ? "#C8F55A" : "#a1a1aa", textTransform: "uppercase", letterSpacing: "0.8px" }}>{label}</div>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {showColor && (
          <div style={{ width: 20, height: 20, borderRadius: 6, background: showColor, border: "1.5px solid #3f3f46", boxShadow: `0 0 12px ${showColor}33` }} />
        )}
        <div style={{ fontSize: 13, fontWeight: 600, color: isHighlight ? "#4ADE80" : "#d4d4d8", fontFamily: showColor ? "monospace" : "inherit" }}>{value}</div>
      </div>
    </div>
  );
}
